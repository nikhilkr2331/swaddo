...
import { Phone, Lock, ArrowRight, Loader2, Store } from "lucide-react";
    ...
    <div className="min-h-screen flex items-center justify-center p-6 bg-bg-main relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-bg-alt rounded-3xl p-8 shadow-xl border border-border-subtle relative z-10">
        <div className="mb-8 text-center">
          <div className="w-16 h-16 rounded-2xl bg-accent flex items-center justify-center mx-auto mb-4 text-white shadow-lg">
            <Store size={32} />
          </div>
          <h1 className="text-2xl font-heading font-bold text-text-primary mb-2">Merchant Portal</h1>
          <p className="text-text-muted text-sm">Sign in to manage your stall and orders.</p>
        </div>
        ...
        {step === 1 ? (
          <form onSubmit={requestOtp} className="space-y-6">
            <div>
              <label className="block text-sm font-semibold text-text-primary mb-2">Registered Mobile</label>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-bg-main border border-border-subtle rounded-xl py-3.5 pl-[85px] pr-4 outline-none focus:border-accent transition-colors font-medium text-text-primary"
                  maxLength={10}
                />
              </div>
            </div>
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-accent hover:bg-yellow-600 text-white font-bold py-3.5 rounded-xl transition-colors shadow-md flex items-center justify-center gap-2"
            >
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-bg-main border border-border-subtle rounded-xl py-3.5 pl-12 pr-4 outline-none focus:border-accent transition-colors font-medium text-text-primary tracking-widest"
                  maxLength={4}
                />
              </div>
              <p className="text-xs text-text-muted mt-3 text-center">
                OTP sent to +91 {phone}. <button type="button" onClick={() => setStep(1)} className="text-accent font-bold hover:underline">Edit</button>
              </p>
            </div>
            <button
              type="submit"
